...
class PonctuationPreprocessorUseCase:
    ...
    def execute(self, voie_obj: Voie, ponctuations: list) -> Voie:
        chaine_decoupee = self.separate_words_with_apostrophe_and_supress_ponctuation_use_case.execute(voie_obj.label_raw, ponctuations)
        new_label_preproc = self.suppress_ponctuation_in_words_use_case.execute(chaine_decoupee, ponctuations)
