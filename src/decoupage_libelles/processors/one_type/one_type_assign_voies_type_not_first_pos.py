...
class OneTypeAssignNotFirstPos():
    def __init__(self, voie: DecoupageVoie):
        self.voie = voie

    def assign_if_type_is_long(self):
        if (self.voie.type_is_long(1) and
                not self.voie.has_adj_det_before_type(1) and
                not self.voie.has_type_in_last_pos()):
            # compl + 1er type + lib
            # test = VoieType('LE BAS FAURE RUE DE TOUL', ['LE', 'BAS', 'FAURE', 'RUE', 'DE', 'TOUL'], ['RUE'], [3], ['DET', 'PROPN', 'PROPN', 'NOUN', 'ADP', 'PROPN'])
            self.voie.assign_compl_type_lib(1)
