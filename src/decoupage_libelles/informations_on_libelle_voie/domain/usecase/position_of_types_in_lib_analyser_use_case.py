...
class PositionOfTypesInLibAnalyserUseCase:
    ...
    def execute(self, infovoie: InfoVoie) -> InfoVoie:
        ...
        if type_ordered_second and type_ordered_second.position_start == 1:
            infovoie.has_type_in_second_pos = True
        
        type_ordered_last = self.order_type_in_lib_use_case.execute(infovoie, -1)
        if type_ordered_last and type_ordered_last.position_end==len(infovoie.label_preproc)-1:
            infovoie.has_type_in_last_pos = True
