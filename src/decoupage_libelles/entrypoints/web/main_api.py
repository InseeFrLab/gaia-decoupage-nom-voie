...
class VoiesData(BaseModel):
    ...
    def process(self) -> List[Dict[str, Dict[str, str]]]:
        self.list_labels_voies = list(set(self.list_labels_voies))

        typevoiedecoupagelauncher: TypeVoieDecoupageLauncher = TypeVoieDecoupageLauncher()
        voies_processed = typevoiedecoupagelauncher.execute(voies_data=self.list_labels_voies)
...
async def analyse_libelles_voies(voies_data: VoiesData):
    return {"reponse": voies_data.process()}
